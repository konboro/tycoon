...
export function renderGuildTab(container) {
    ...
    if (!playerGuildId) {
            ...
            <div class="p-4 space-y-4 h-full flex flex-col">
                <div class="bg-gray-800/90 p-4 rounded-xl border border-indigo-500/30 shadow-lg">
                    <div class="flex justify-between items-start">
                        <div><h2 class="text-2xl font-bold text-white">${myGuild.name}</h2><p class="text-xs text-gray-400">Lider: ${myGuild.leader}</p></div>
                        <div class="text-right"><div class="text-xs text-gray-400">Skarbiec</div><div class="text-xl font-mono font-bold text-yellow-400">${fmt(myGuild.bank)} VC</div><div class="text-xs text-green-400">+${fmt(totalIncome)} VC/min</div></div>
                    </div>
                ...
                <div class="flex-grow overflow-y-auto space-y-4">
                    <div><h3 class="font-bold text-white mb-2">Posiadane Elektrownie</h3><div id="guild-owned-list" class="space-y-2"></div></div>
                    <div><h3 class="font-bold text-white mb-2">Rynek Inwestycyjny</h3><div id="guild-shop-list" class="space-y-2"></div></div>
                </div>
                <div class="h-32 bg-gray-900/50 rounded-lg border border-gray-700 flex flex-col"><div id="guild-chat-messages" class="flex-grow overflow-y-auto p-2 text-xs"></div><div class="flex p-1 border-t border-gray-700"><input id="chat-message-input" class="flex-grow bg-transparent text-white px-2 outline-none" placeholder="Czat..."><button id="send-chat-msg-btn" class="text-blue-400 px-2">></button></div></div>
            </div>`;
            
        const ownedDiv = $('guild-owned-list'); const shopDiv = $('guild-shop-list');
        ...
        const chat = $('guild-chat-messages'); (myGuild.chat || []).forEach(m => { chat.innerHTML += `<div><span class="text-blue-400 font-bold">${m.sender}:</span> <span class="text-gray-300">${m.message}</span></div>`; });
    }
}
