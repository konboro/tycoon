// js/ui.js - WERSJA FINALNA (Z BEZPIECZNYM KUPNEM)
...
function openLootbox(boxType) {
    ...
    setTimeout(() => {
        ...
        if (prizePool.length > 0) {
            ...
            // Zapis do bazy (bez RPC dla skrzynek, bo to nagroda)
            (async () => {
            ...
            (async () => {
                const user = (await supabase.auth.getUser()).data.user;
                if(user) await supabase.from('profiles').update({ wallet: state.wallet }).eq('id', user.id);
            })();
...
export function renderVehicleList(container) {
    ...
    const filtered = listSource.filter(v => {
        ...
        // Ukryj zajęte w sklepie (jeśli są w globalTaken i nie są moje)
...
export function renderCharts(container) {
    ...
    const stats = {}; ['plane', 'train', 'tube', 'bus', 'bike', 'river-bus', 'tram'].forEach(type => { stats[type] = { count: 0, totalEarnings: 0, totalKm: 0, totalValue: 0, estimatedVcPerHour: 0 }; }); 
    for (const key in state.owned) { const vehicle = state.owned[key]; const type = vehicle.type; if (stats[type]) { stats[type].count++; stats[type].totalEarnings += vehicle.earned_vc || 0; stats[type].totalKm += vehicle.odo_km || 0; stats[type].totalValue += config.basePrice[type] || 0; } }

    for (const type in stats) { const data = stats[type]; if (data.count > 0) { const el = document.createElement('div'); el.className = 'bg-gray-800/50 p-3 rounded-lg border border-gray-700/50 mb-2'; el.innerHTML = `<div class="flex items-center gap-2 font-bold mb-2"><span class="text-xl">${ICONS[type]}</span><span>${type} (${data.count})</span></div><div class="grid grid-cols-2 gap-x-4 gap-y-1 text-xs"><span class="text-gray-400">Wartość:</span><span class="text-right font-medium">${fmt(data.totalValue)} VC</span><span class="text-gray-400">Zarobek:</span><span class="text-right font-medium">${fmt(data.totalEarnings)} VC</span></div>`; summaryContainer.appendChild(el); } }
      ...
      if (buyTarget) { 
          ...
          (async () => {
              ...
              if (data.success) {
                  ...
                  state.owned[key] = { 
                      ...vehicleData, odo_km: 0, earned_vc: 0, wear: 0, purchaseDate: new Date().toISOString(), customName: null, level: 1, totalEnergyCost: 0, earningsLog: [], serviceHistory: [] 
                  }; 
